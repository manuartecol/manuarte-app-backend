...
export class OpenAIService {
	...
	detectIntentWithAI = async (
		...
		const showMoreNote = awaitingMoreProducts
			? '\nNota: hay más productos disponibles que no se le han mostrado al cliente todavía. IMPORTANTE: si el cliente pregunta por un tipo de producto diferente o específico (ej: "qué ceras tienes", "tienen fragancias", "qué parafinas hay"), clasifica como "search_product" con el término buscado, NO como "show_more".\n'
			: '';
		...
		const showMoreInstruction =
			hasActiveProductList || (activeProducts && activeProducts.length > 0)
				? `  - "show_more": el cliente pregunta si hay más opciones, más productos, más variantes, o pide ver más (incluyendo frases negativas como "¿no tienen más?", "¿no hay más?", "¿solo eso tienen?", "¿solo tienes esa?", "¿nada más?", "¿no tienes otra?"). IMPORTANTE: clasifica como "search_product" (NO como "show_more") cuando el cliente: (1) dice "tienes más [nombre de producto]" (ej: "tienes más mechas"); (2) pregunta por un tipo de producto específico con interrogativo (ej: "qué ceras tienes", "qué fragancias hay", "cuáles parafinas tienen", "qué tipos de ceras manejan", "tienen ceras disponibles"). El intent "show_more" SOLO aplica cuando el cliente pide ver más resultados de la búsqueda en curso, sin especificar un tipo de producto diferente.\n`
				: '';
